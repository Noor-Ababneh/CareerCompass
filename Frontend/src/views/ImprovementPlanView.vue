    ...
    <div v-if="source === 'grades'" class="space-y-10">
      ...
      <section class="anim-fade text-center">
        ...
        <h1 class="text-4xl md:text-5xl font-black text-slate-800 mb-3 leading-tight">
           تحليل وضعك الأكاديمي - <span class="text-transparent bg-clip-text bg-gradient-to-r from-amber-600 to-orange-500">{{ getFieldName(field) }}</span>
        </h1>
      ...
      <!-- ============================================================ -->
      <!-- FAILED SUBJECT CRITICAL BANNER - يظهر فقط عند وجود رسوب -->
      <!-- ============================================================ -->
      <section v-if="hasFailedSubject" class="anim-fade">
        <div class="relative overflow-hidden rounded-[2rem] border-2 border-rose-300 bg-gradient-to-br from-rose-50 via-red-50 to-rose-100 shadow-xl shadow-rose-500/15 p-8 md:p-10">
          <!-- Decorative pulse ring -->
          <div class="absolute top-6 right-6 w-14 h-14 rounded-full bg-rose-200/60 animate-ping pointer-events-none"></div>
          <div class="absolute top-6 right-6 w-14 h-14 rounded-full bg-rose-300/80 flex items-center justify-center text-2xl z-10">🚫</div>

          <div class="pr-20">
            <h2 class="text-2xl md:text-3xl font-black text-rose-800 mb-3 leading-tight">
              يوجد مادة راسبة — القبول الجامعي موقوف حالياً
            </h2>
            <p class="text-rose-700 font-bold text-base leading-relaxed mb-6">
              الرسوب في أي مادة أساسية يمنع القبول الجامعي بصورة كاملة في الجامعات الأردنية
              الحكومية والخاصة على حدٍّ سواء، بغض النظر عن المعدل العام أو باقي العلامات.
              معالجة المواد الراسبة هي أولوية قصوى قبل أي تفكير في التخصصات أو المعدلات.
            </p>

            <!-- Failed subjects list -->
            <div class="flex flex-wrap gap-3 mb-6">
              <div v-for="subj in failedSubjectsList" :key="subj.key"
                class="flex items-center gap-2 bg-white border-2 border-rose-200 rounded-2xl px-4 py-2.5 shadow-sm">
                <span class="text-xl">{{ subj.icon }}</span>
                <div>
                  <div class="text-sm font-black text-rose-700">{{ subj.name }}</div>
                  <div class="text-xs font-bold text-rose-400 font-mono">{{ subj.score }}% — الحد الأدنى للنجاح: 50%</div>
                </div>
              </div>
            </div>
            ...
            <div class="grid sm:grid-cols-3 gap-4">
              <div class="bg-white/70 border border-rose-200 rounded-2xl p-4 text-center">
                <div class="text-2xl mb-2">📌</div>
                <div class="text-sm font-black text-rose-700 mb-1">الخطوة الأولى</div>
                <div class="text-xs text-rose-600 font-bold leading-relaxed">راجع كل مادة راسبة فردياً واستعن بمعلم خصوصي فوراً</div>
              </div>
              <div class="bg-white/70 border border-rose-200 rounded-2xl p-4 text-center">
                <div class="text-2xl mb-2">📅</div>
                <div class="text-sm font-black text-rose-700 mb-1">الجدول الزمني</div>
                <div class="text-xs text-rose-600 font-bold leading-relaxed">خصص 70% من وقت دراستك للمواد الراسبة حتى الامتحان</div>
              </div>
              <div class="bg-white/70 border border-rose-200 rounded-2xl p-4 text-center">
                <div class="text-2xl mb-2">🎯</div>
                <div class="text-sm font-black text-rose-700 mb-1">الهدف الآني</div>
                <div class="text-xs text-rose-600 font-bold leading-relaxed">النجاح في المواد الراسبة أولاً — كل هدف آخر يأتي بعده</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section class="anim-fade-d1 bg-white/80 backdrop-blur-xl border border-white rounded-[2.5rem] overflow-hidden relative shadow-xl shadow-amber-500/5">
        ...
        <div class="p-8 md:p-12 flex flex-col md:flex-row items-center gap-12">
          ...
          <div class="flex-1 text-center md:text-right">
            ...
            <!-- Failed subject warning inside GPA card -->
            <div v-if="hasFailedSubject" class="mt-6 p-4 rounded-2xl bg-rose-50 border-2 border-rose-100 flex items-start gap-3">
              ...
              <p class="text-sm text-rose-700 font-bold leading-relaxed text-right">
                يوجد مادة راسبة — جميع بيانات القبول أدناه مجمّدة. اعالج الرسوب أولاً.
              </p>
            </div>

            <!-- ============================================================ -->
            <!-- ADMISSION THRESHOLDS: مخفية عند الرسوب، ظاهرة عند عدمه -->
            <!-- ============================================================ -->
            <div v-if="!hasFailedSubject" class="mt-8 grid grid-cols-3 gap-4">
            ...
            <!-- بديل عند الرسوب: بطاقات موقوفة -->
            <div v-else class="mt-8 grid grid-cols-3 gap-4">
              <div v-for="label in ['قبول خاص','قبول حكومي','تخصصات منافسة']" :key="label"
                class="bg-rose-50/60 border border-rose-100 p-3 rounded-2xl text-center shadow-sm opacity-60">
                <div class="text-xl font-black text-rose-300">—</div>
                <div class="text-[10px] text-rose-400 font-black mt-1">{{ label }}</div>
                <div class="text-[10px] text-rose-300 font-bold">موقوف</div>
              </div>
            </div>
      ...
      <section v-if="stage === 'senior'" class="anim-fade-d2">
        ...
        <!-- ============================================================ -->
        <!-- MAJORS BLOCKED: تغطية كاملة عند وجود رسوب -->
        <!-- ============================================================ -->
        <div v-if="hasFailedSubject" class="relative">
          <!-- Blur overlay with message -->
          <div class="rounded-3xl border-2 border-rose-200 bg-rose-50/80 backdrop-blur-sm p-10 text-center">
            <div class="text-5xl mb-4">🔒</div>
            <h3 class="text-xl font-black text-rose-700 mb-3">التخصصات موقوفة — يوجد مادة راسبة</h3>
            <p class="text-rose-600 text-sm font-bold max-w-lg mx-auto leading-relaxed mb-6">
              لا يمكن عرض خيارات التخصصات الجامعية لأن الرسوب في أي مادة أساسية يمنع القبول
              الجامعي كلياً. بعد معالجة المواد الراسبة وتحقيق النجاح فيها، ستظهر هنا جميع
              التخصصات المتاحة بناءً على معدلك الجديد.
            </p>
            <!-- Faded preview of majors behind the message -->
            <div class="opacity-20 pointer-events-none space-y-2 mt-4">
              <div v-for="major in (majorsDB[field] || []).slice(0,3)" :key="major.name"
                class="bg-white border border-slate-200 rounded-xl p-3 text-right">
                <span class="text-sm font-black text-slate-600">{{ major.name }}</span>
              </div>
            </div>
          </div>
        </div>

        <!-- Normal majors display (no failed subject) -->
        <div v-else>
        ...
        <div class="space-y-6">
          <div v-for="(item, idx) in sortedImprovementPlan" :key="idx"
            ...
            <div class="p-6 md:p-8 pr-8">
              <div class="flex flex-col md:flex-row justify-between gap-6 mb-6">
                <div class="flex-1">
                  <div class="flex items-center gap-3 mb-3">
                    ...
                    <!-- Failed badge -->
                    <span v-if="item.isFailed" class="text-[11px] font-black px-3 py-1 rounded-full bg-rose-600 text-white shadow-sm animate-pulse">🚫 راسب</span>
                  </div>
              ...
              <!-- Failed subject urgent action box -->
              <div v-if="item.isFailed" class="mb-5 p-4 rounded-2xl bg-rose-50 border-2 border-rose-200">
                <div class="flex items-center gap-2 mb-2">
                  <span class="text-lg">🚨</span>
                  <span class="text-sm font-black text-rose-700">إجراء عاجل مطلوب — هذه المادة تمنع القبول الجامعي</span>
                </div>
                <p class="text-xs text-rose-600 font-bold leading-relaxed">
                  يجب رفع علامتها فوق 50% كأولوية مطلقة قبل أي هدف آخر.
                  لا تنتقل لمراجعة المواد الأخرى إلا بعد تخصيص ساعتين يومياً لهذه المادة على الأقل.
                </p>
              </div>
      ...
      <section v-if="learningPlan" class="anim-fade-d2 mt-16 bg-white/90 backdrop-blur-xl border border-white rounded-[3rem] overflow-hidden relative shadow-2xl shadow-amber-500/10 print:break-before-page">
        ...
        <div class="p-8 md:p-14 relative z-10">
            ...
            <div class="space-y-10">
              <div v-for="(phase, idx) in learningPlan.phases" :key="idx" class="relative flex gap-6 md:gap-8 group">
                ...
                <div class="flex-1 pb-2">
                  ...
                  <!-- Resources per phase if available -->
                  <div v-if="phase.resources?.length" class="mt-4 flex flex-wrap gap-2">
                    <span class="text-[10px] font-black text-slate-400 uppercase self-center">أدوات الخطوة:</span>
                    <a v-for="res in phase.resources" :key="res.name" :href="res.url" target="_blank"
                      class="flex items-center gap-1.5 bg-amber-50 px-3 py-1.5 rounded-lg border border-amber-200 hover:border-amber-400 hover:text-amber-700 text-xs font-black text-amber-600 transition-all shadow-sm print:hidden">
                      <span>{{ res.icon }}</span> {{ res.name }}
                    </a>
                  </div>
                </div>
          ...
          <div v-if="learningPlan.metrics" class="mt-12 pt-8 border-t border-slate-100 relative z-10">
            <h4 class="text-sm font-black text-slate-500 uppercase tracking-wider mb-4 text-center">مؤشرات النجاح الأسبوعية</h4>
            <div class="grid sm:grid-cols-3 gap-4">
              <div v-for="metric in learningPlan.metrics" :key="metric.label"
                class="bg-amber-50/50 border border-amber-100 rounded-2xl p-4 text-center">
                <div class="text-2xl mb-1">{{ metric.icon }}</div>
                <div class="text-xs font-black text-amber-700">{{ metric.label }}</div>
                <div class="text-[10px] text-amber-500 font-bold mt-1">{{ metric.value }}</div>
              </div>
            </div>
          </div>
...
const majorsDB = {
  Health: [
    ...
    { name: 'صيدلة', minGPA: 80, govMin: 80, competitive: true },
    { name: 'تمريض', minGPA: 70, govMin: 70, competitive: false },
    { name: 'علوم طبية مخبرية', minGPA: 70, govMin: 70, competitive: false },
    { name: 'العلوم الطبية المساندة: الأشعة, التخدير, العلاج الطبيعي, العلاج التنفسي', minGPA: 70, govMin: 70, competitive: false }
  ],
  EngineeringIT: [
    ...
    { name: 'نظم معلومات', minGPA: 75, govMin: 75, competitive: true }
  ],
  Business: [
    ...
    { name: 'اقتصاد', minGPA: 65, govMin: 65, competitive: false },
    { name: 'مالية ومصرفية', minGPA: 65, govMin: 65, competitive: false }
  ],
  Humanities: [
...
function getSubjectIcon(k) {
  return subjectMeta[k]?.icon || '📚'
}

// UNIFIED LEVEL FUNCTION
...
function buildImprovementItem(key, score) {
  ...
  if (score >= 93) {
    ...
    diagnosis = 'المستوى: راسب (أقل من 50%). الوضع يحتاج تدخلاً فورياً وجدياً. الهدف العاجل: الوصول لعلامة النجاح (50%) أولاً — القبول الجامعي يتوقف على هذا.'
  }
  ...
  const timelineMap = {
    fail: '8–12 أسبوعاً — هدف أولي: النجاح (50%)',
    weak: '6–8 أسابيع — هدف: تجاوز حد القبول (65%)',
    good: '4–6 أسابيع — هدف: الدخول في النطاق التنافسي',
    vgood: '2–4 أسابيع — هدف: الامتياز',
...
// =============================================
// FAILED SUBJECTS LIST - for the critical banner
// =============================================
const failedSubjectsList = computed(() => {
  return Object.entries(grades)
    .filter(([, s]) => Number(s) < 50)
    .map(([k, s]) => ({
      key: k,
      name: getSubjectName(k),
      icon: getSubjectIcon(k),
      score: Number(s)
    }))
...
// =============================================
// hasFailedSubject: true if ANY subject < 50
// =============================================
...
// =============================================
// LEARNING PLAN - Enhanced with resources per phase and metrics
// =============================================
const learningPlan = computed(() => {
  ...
  if (gpa < 50 || hasFailedSubject) return {
    ...
    note: failedSubjects.length ? `⚠️ مواد راسبة حرجة: ${failedSubjects.join('، ')}` : 'جميع المواد تحتاج تعزيزاً عاجلاً',
    quote: 'التعلم الفعّال لا يبدأ بالحفظ، بل بتشخيص الفجوة ومعالجتها بمنهجية.',
    metrics: [
      { icon: '⏰', label: 'ساعات الدراسة اليومية', value: '4 ساعات صافية كحد أدنى' },
      { icon: '📋', label: 'هدف الأسبوع', value: 'تحسن 5% في كل مادة راسبة' },
      { icon: '✅', label: 'معيار الانتقال', value: '50%+ في التدريب قبل الانتقال' }
    ],
    phases: [
      {
        ...
        resources: [
          { name: 'Khan Academy عربي', url: 'https://ar.khanacademy.org', icon: '🌐' },
          { name: 'منصة إدراك', url: 'https://www.edraak.org', icon: '🎓' }
        ],
        steps: [
          'طبّق اختباراً تشخيصياً ذاتياً في كل مادة لتحديد المفاهيم الغائبة بالضبط - لا تخمن، اختبر نفسك فعلياً.',
          'رتّب المواد حسب الأثر على المعدل. ابدأ بالمادة الراسبة التي لها أكبر وزن.',
          ...
          'للمواد الراسبة تحديداً: خصص 70% من وقتك لها حصراً في هذه المرحلة.',
          'استخدم تطبيق Forest أو ساعة رملية فعلية للتحكم بوقت الدراسة بلا تشتيت.'
        ]
      },
      {
        name: 'المرحلة الثانية: إعادة البناء المعرفي',
        duration: 'الأسابيع 3-8',
        target: 'تجاوز حد النجاح 50% في كل مادة راسبة',
        resources: [
          { name: 'قناة الأوائل', url: 'https://awa2el.net', icon: '▶️' },
          { name: 'منصة وتد - سامر رشاد', url: 'https://watad.me', icon: '🎓' },
          { name: 'Anki للمراجعة', url: 'https://apps.ankiweb.net', icon: '🃏' }
        ],
        steps: [
          'استخدم أسلوب التعلم التراكمي التصاعدي: لا تنتقل لمفهوم جديد قبل إتقان السابق بنسبة 80%.',
          'طبّق أسلوب الاسترجاع النشط (Active Recall): أغلق الكتاب وحاول كتابة ما تعلمته - يرسّخ المعلومات 3 أضعاف الإعادة.',
          ...
          'استخدم Anki لحفظ المفاهيم الأساسية بنظام التكرار المتباعد - 15 دقيقة يومياً كافية.'
        ]
      },
      {
        ...
        resources: [
          { name: 'سنوات وزارية سابقة', url: 'https://awa2el.net', icon: '📄' },
          { name: 'مجموعات الطلاب - تيليغرام', url: 'https://t.me', icon: '💬' }
        ],
        steps: [
          ...
          'انضم لمجموعة دراسية واشرح مواد لزملائك — الشرح يكشف ثغراتك قبل الامتحان.',
          'في آخر أسبوعين: مراجعة خفيفة فقط، وادرس تقنية إدارة الوقت داخل الامتحان.'
        ]
      }
    ]
  }
  ...
  if (gpa < 65) return {
    ...
    metrics: [
      { icon: '📈', label: 'هدف التحسن الأسبوعي', value: '+3% في كل مادة ضعيفة' },
      { icon: '📝', label: 'نماذج وزارية', value: '3 نماذج على الأقل أسبوعياً' },
      { icon: '🎯', label: 'هدف الشهر الأول', value: 'تجاوز 65% في كل مادة' }
    ],
    phases: [
      {
        ...
        steps: [
          ...
          'استخدم جدول Google Calendar أو Notion لتتبع خطتك يومياً بشكل مرئي.',
          'حدد ساعة ذروة تركيزك اليومية (صباحاً أم مساءً؟) وخصصها للمادة الأصعب.'
        ]
      },
      {
        ...
        resources: [
          { name: 'منصة وتد', url: 'https://watad.me', icon: '🎓' },
        ...
        steps: [
          ...
          'استخدم Quizlet لإنشاء بطاقات مراجعة تفاعلية لكل وحدة - يجعل المراجعة أسرع.'
        ]
      },
      {
        ...
        resources: [
          { name: 'نماذج وزارية PDF', url: 'https://awa2el.net', icon: '📄' },
          { name: 'Notion - تتبع التقدم', url: 'https://notion.so', icon: '📋' }
        ],
        steps: [
          ...
          'تعلم تقنية "ابدأ بالأسهل": حل الأسئلة السهلة أولاً يبني ثقتك ويضمن درجاتها.',
          'قبل الامتحان بيوم: استرح، تغذَّ جيداً، نَم 8 ساعات - الجسم جزء من الأداء.'
        ]
      }
    ]
  }
  ...
  if (gpa < 80) return {
    ...
    metrics: [
      { icon: '🏆', label: 'الهدف الرئيسي', value: 'تجاوز 80% للدخول في النطاق التنافسي' },
      { icon: '📊', label: 'معدل التدريب المستهدف', value: '80%+ في كل محاكاة' },
      { icon: '⚡', label: 'نماذج وزارية أسبوعية', value: '2 نموذج كامل على الأقل' }
    ],
    phases: [
      {
        ...
        resources: [
          { name: 'قناة الأوائل', url: 'https://awa2el.net', icon: '▶️' },
          { name: 'منصة وتد', url: 'https://watad.me', icon: '🎓' },
          { name: 'Desmos', url: 'https://desmos.com', icon: '📊' }
        ],
        steps: [
          ...
          'لكل وحدة ضعيفة: شاهد شرح فيديو كامل ثم حل التمارين مباشرة دون توقف. لا تفصل الشرح عن التطبيق.',
          'أنشئ "خريطة الوحدات" التي تُظهر ترابطها - فهم العلاقات يجعل المراجعة أسرع بكثير.'
        ]
      },
      {
        ...
        resources: [
          { name: 'Quizlet', url: 'https://quizlet.com', icon: '📚' },
          { name: 'Notion لتتبع التقدم', url: 'https://notion.so', icon: '📋' },
          { name: 'Forest - التركيز', url: 'https://www.forestapp.cc', icon: '🌲' }
        ],
        steps: [
          ...
          'اعمل مخططاً بيانياً لتقدمك الأسبوعي في كل مادة - التحسن المرئي محفز نفسياً مثبت علمياً.',
          'إذا تكرر نفس الخطأ أكثر من مرة: ارجع لشرح المفهوم الأساسي - الخطأ المتكرر هو عرض لمشكلة أعمق.'
        ]
      },
      {
        ...
        resources: [
          { name: 'سنوات وزارية 2019-2024', url: 'https://awa2el.net', icon: '📄' }
        ],
        steps: [
          ...
          'يوم قبل الامتحان: لا دراسة مكثفة - فقط مراجعة سريعة للـ "ورقة الخلاصة الشخصية".',
          'في الامتحان: اقرأ كل الأسئلة أولاً (3 دقائق) قبل البدء — يساعدك على توزيع الوقت ذهنياً.'
        ]
      }
    ]
  }
  ...
  if (gpa < 93) return {
    ...
    quote: '"Excellence is not a destination you arrive at once, but a standard you keep choosing."',
    metrics: [
      { icon: '🎯', label: 'هدف كل تدريب', value: '90%+ والاقتراب من الامتياز' },
      { icon: '⏱️', label: 'تدريب الوقت', value: 'حل الامتحان في 80% من الوقت' },
      { icon: '📉', label: 'تقليل الأخطاء', value: 'صفر أخطاء إجرائية في التدريب' }
    ],
    phases: [
      {
        ...
        resources: [
          { name: 'سنوات وزارية 2020-2024', url: 'https://awa2el.net', icon: '📄' },
          { name: 'Wolfram Alpha', url: 'https://wolframalpha.com', icon: '🔢' }
        ],
        steps: [
          ...
          'اعمل جدولاً: كل مادة × نوع الخطأ الأكثر تكراراً × الحل المقترح. هذا الجدول هو قائدك.',
          'اسأل معلمك تحديداً: ما الأسئلة التي يراها الممتحنون متميزة؟ هذه المعلومة تساوي ذهباً.'
        ]
      },
      {
        ...
        resources: [
          { name: 'Notion لتتبع الأداء', url: 'https://notion.so', icon: '📋' },
          { name: 'Khan Academy - مستوى متقدم', url: 'https://ar.khanacademy.org', icon: '🌐' }
        ],
        steps: [
          ...
          'ساعد زميلاً أقل مستوى في مادة تتقنها - شرح المادة يكشف ثغراتك الخفية.'
        ]
      },
      {
        ...
        resources: [
          { name: 'Coursera - تحضير جامعي', url: 'https://www.coursera.org', icon: '🎓' },
          { name: 'LinkedIn - الملف المهني', url: 'https://linkedin.com', icon: '💼' }
        ],
        steps: [
          ...
          'أنشئ ملفك على LinkedIn الآن - سجّل إنجازاتك الأكاديمية وابدأ التواصل مع طلاب سنة أولى.',
          'تصفح Coursera وحدد دورة تمهيدية في تخصصك لتبدأها في الصيف - الأفضلية لمن يبدأ مبكراً.'
        ]
      }
    ]
  }
  ...
  return {
    ...
    quote: '"The secret to sustained excellence is not working harder, but working smarter with deliberate practice."',
    metrics: [
      { icon: '🏅', label: 'الهدف', value: 'الحفاظ على الأداء بأقل جهد ممكن' },
      { icon: '🎓', label: 'الجاهزية الجامعية', value: 'ابدأ مقررات السنة الأولى مبكراً' },
      { icon: '🌍', label: 'المنح والفرص', value: 'ابحث الآن — معظمها تتطلب تقديماً مبكراً' }
    ],
    phases: [
      {
        ...
        resources: [
          { name: 'Forest - إدارة الوقت', url: 'https://www.forestapp.cc', icon: '🌲' },
          { name: 'Anki للمراجعة الخفيفة', url: 'https://apps.ankiweb.net', icon: '🃏' }
        ],
        steps: [
          ...
          'قبل كل امتحان تجريبي: تأكد من النوم جيداً - الأداء المعرفي ينخفض 20% عند الحرمان من النوم.',
          'شارك في أي مسابقة علمية مدرسية — الإنجازات خارج الصف تُقوّي ملف القبول الجامعي.'
        ]
      },
      {
        ...
        steps: [
          ...
          'لتخصصات التقنية: ابدأ مشروعاً بسيطاً على GitHub — ملف مشاريع حقيقي يفوق الشهادات.'
        ]
      }
    ]
  }
...
function getGPASummaryTitle(g) {
  if (hasFailedSubject) return 'يوجد مادة راسبة — القبول موقوف بغض النظر عن المعدل.'
...
function getGPADetailedMessage(g, f) {
  if (hasFailedSubject) return 'يوجد مادة راسبة. الرسوب في أي مادة أساسية يوقف القبول الجامعي كلياً في جميع الجامعات الأردنية، حكومية وخاصة، بصرف النظر عن المعدل العام. المطلوب أولاً: معالجة المادة الراسبة وتجاوز حد النجاح (50%).'

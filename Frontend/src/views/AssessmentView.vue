<template>
  <div class="min-h-screen flex items-center justify-center p-4 md:p-8 bg-[#FDFCF9] font-sans selection:bg-amber-500 selection:text-white relative overflow-hidden" dir="rtl">
    <div class="absolute top-8 left-8 z-50 flex items-center gap-4">
  <button @click="$router.push('/')" class="flex items-center gap-2 text-slate-500 hover:text-indigo-600 transition-colors font-bold group">
    <span class="text-xl group-hover:-translate-x-1 transition-transform duration-300">←</span>
    <span>الرئيسية</span>
  </button>
  
  <button @click="$router.push('/dashboard')" 
    class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded-xl font-bold text-sm transition-all shadow-md hover:-translate-y-0.5 flex items-center gap-2">
    <span>📊</span>
    ...
    <div class="w-full relative z-10 transition-all duration-700 flex items-center justify-center" :class="currentScreen === 'intro' ? 'max-w-3xl' : 'max-w-2xl'">
      
      <!-- Floating Avatar - تم تعديل التموضع ليكون في المنتصف عمودياً وإلى يسار البوكس -->
      <div v-if="currentScreen === 'phaseIntro' && (currentPhase === 2 || currentPhase === 3) && userGender" class="fixed left-auto right-[calc(50%+360px)] top-1/2 -translate-y-1/2 z-20 hidden xl:block animate-float pointer-events-none">
        <div class="border-[3px] border-amber-300 rounded-3xl overflow-hidden shadow-2xl shadow-amber-500/20 bg-white p-2 w-[300px]">
          <img :src="avatarSrc" class="w-full h-auto rounded-2xl" alt="Student Avatar" />
        </div>
      </div>

      <div class="bg-white/85 backdrop-blur-xl border border-white/60 rounded-[2.5rem] shadow-[0_20px_60px_-15px_rgba(245,158,11,0.15)] overflow-hidden relative min-h-[450px] w-full">

        ...
        <div v-if="selectedField" class="mb-6 bg-white/80 backdrop-blur-xl border border-white/60 shadow-xl shadow-amber-500/10 rounded-[2rem] p-6 md:p-8 relative overflow-hidden">
          ...
          <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
            <div class="step-header">
              ...
              <h2 class="font-bold text-slate-800 text-lg">
                {{ academicStage === 'junior' ? 'أدخل علاماتك المدرسية لتقييم جاهزيتك' : 'أدخل علامات التوجيهي' }}
              </h2>
            </div>
          ...
          <div class="space-y-8">
            <div v-for="(label, key) in currentSubjects" :key="key" class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100/50 hover:border-amber-200 transition-colors">
              ...
              <div class="relative py-2">
                ...
                <div class="absolute top-6 w-full pointer-events-none">
  <div class="absolute text-[10px] text-rose-400 font-black translate-x-1/2" style="right:50%">50</div>
  <div class="absolute text-[10px] text-amber-500 font-black translate-x-1/2" style="right:65%">65</div>
  <div class="absolute text-[10px] text-sky-500 font-black translate-x-1/2" style="right:75%">75</div>
  <div class="absolute text-[10px] text-emerald-500 font-black translate-x-1/2" style="right:85%">85</div>
</div>
              </div>
            </div>
          </div>
...
async function submit() {
  ...
  try {
    ...
    console.log("✅ تم الحفظ في الباك إند بنجاح!", response.data);
    ...
    errorMessage.value = 'حدث خطأ في الاتصال بالباك إند.';
  }
}
</script>
